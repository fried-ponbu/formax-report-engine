...
Read **`DISTRIBUTION.md`** for installation instructions — bare-metal
(`.deb` + systemd) or Docker — and a walkthrough of the included example
template and sample payload.

Read **`TEMPLATE_GUIDE.md`** for the full template authoring reference:
...
Import **`FormaX-Reporting-Engine.postman_collection.json`** into Postman
for a ready-to-use collection with every endpoint pre-configured.
...
![FormaX Dashboard](dashboard-preview.png)
